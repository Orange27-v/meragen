...
// Meerah: every call goes to our own API on the same origin.
//
// Upstream this pointed at api.muapi.ai (directly, or via a Next.js proxy).
// Our API implements the same wire contract — POST /api/v1/{model} returning a
// request_id, then GET /api/v1/predictions/{id}/result — but it also charges
// credits, enforces the margin floor and refunds failures, none of which the
// browser should be trusted with. The vendor key never leaves our server.
//
// The empty base leaves paths as `/api/v1/...` on our own origin, so there is
// no CORS and no key in the bundle.
const BASE_URL = '';
const PROXY_WF_BASE = '/api/workflow';
const FILE_UPLOAD_TIMEOUT_MS = 300_000;
const FILE_UPLOAD_PENDING_PROGRESS = 99;

// The `x-api-key` header below carries a Meerah session token. Keeping the
// header name means none of the studio components needed changing.
