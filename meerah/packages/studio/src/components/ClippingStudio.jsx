...
// Inline SVG Icons
// ---------------------------------------------------------------------------
const ScissorsIcon = ({ className = "text-[var(--chalk)]" }) => (
...
const ErrorToast = ({ toastInstance, message }) => (
  <div
    className={`pointer-events-auto flex w-[340px] max-w-[calc(100vw-32px)] items-start gap-3 rounded-xl border border-red-400/40 bg-[var(--surface)] px-3.5 py-3 text-[13px] text-[var(--chalk)] shadow-[0_10px_30px_rgba(0,0,0,0.15)] transition-all duration-200 ${
  ...
  >
    ...
    <span className="min-w-0 flex-1 py-1 font-medium leading-5 text-[var(--chalk)]">{message}</span>
    <button
      type="button"
      onClick={() => dismissErrorToast(toastInstance.id)}
      className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--steel)] transition-colors hover:bg-[var(--slab-hi)] hover:text-[var(--chalk)] focus:outline-none focus:ring-1 focus:ring-zinc-300"
...
export default function ClippingStudio({
      ...
      <RailSection label="Your video">
          ...
          <div className="flex items-start gap-3 px-1">
            ...
            {!videoUrl && (
              ...
              >
                {videoUploading ? (
                  <div className="flex flex-col items-center justify-center w-full h-full absolute inset-0 bg-[var(--veil)] z-20 backdrop-blur-[1px]">
                    ...
                    <span className={`absolute text-[8px] font-black text-[var(--chalk)] leading-none ${videoProgress >= 100 ? "animate-pulse" : ""}`}>
                      {videoProgress >= 100 ? "..." : `${videoProgress}%`}
                    </span>
                  </div>
                ) : null}

                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--fog)] group-hover:text-[var(--chalk)] transition-colors">
                  ...
                  <PromptPopover className="min-w-[180px] overflow-visible">
                    ...
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        className="w-full h-1 bg-[var(--slab)] rounded appearance-none cursor-pointer accent-primary"
                      />
                    </div>
                  </PromptPopover>
                )}
              ...
              <button
                ...
                className={promptControlClassName({
                  ...
                  className: returnCoordinatesOnly
                    ? "text-[var(--chalk)]"
                    : "text-[var(--iron)] hover:text-[var(--chalk)]",
                })}
              >
  ...
  // ── Render ───────────────────────────────────────────────────────────────
  return (
    <div className="w-full h-full flex flex-col lg:flex-row bg-app-bg text-[var(--chalk)] relative overflow-hidden">
      {/* ── LEFT: SETTINGS RAIL ── */}
      {settingsRail}
                ...
                >
                  <div className="aspect-video bg-[var(--void)] flex items-center justify-center border-b border-[var(--line)] relative overflow-hidden">
                    ...
                    <div className="absolute top-2 right-2 z-10 hidden md:flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-2 bg-[var(--surface)] backdrop-blur-md rounded-full text-[var(--chalk)] hover:bg-red-500 hover:text-[var(--chalk)] transition-all border border-[var(--line)]"
                      >
                        <TrashIcon />
                      </button>
                    </div>
                  ...
                  <div className="p-3 bg-[var(--surface)] backdrop-blur-sm border-t border-[var(--line)] flex-1 flex flex-col justify-between gap-2">
                    <div className="flex flex-col gap-1">
                      <h4 className="text-xs font-bold text-[var(--chalk)] truncate" title={entry.videoUrl.split('/').pop()}>
                        {entry.videoUrl.split('/').pop() || "source_video.mp4"}
                      </h4>
                      <p className="text-[9px] text-[var(--fog)] font-semibold uppercase tracking-wider">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-[var(--line)]">
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold text-[var(--steel)] hover:text-[var(--chalk)] transition-colors"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            ...
            {result.returnCoordinatesOnly ? (
              <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
                ...
                <div className="flex-1 bg-[var(--surface)] border border-[var(--line)] rounded-lg overflow-hidden flex flex-col shadow-2xl relative min-h-[300px] lg:min-h-0">
                  ...
                  <video
                    ...
                    src={result.videoUrl}
                    controls
                    className="w-full flex-1 object-contain bg-[var(--void)]"
                    preload="auto"
                  />
                </div>

                {/* Right Side: Highlights list */}
                <div className="w-full lg:w-[350px] border border-[var(--line)] bg-[color-mix(in_srgb,var(--void)_40%,transparent)] backdrop-blur-md rounded-lg p-5 flex flex-col min-h-[350px] lg:min-h-0">
                  <div className="pb-4 border-b border-[var(--line)] flex items-center justify-between">
                    <h3 className="text-xs font-black text-[var(--chalk)] uppercase tracking-widest">
                      Highlights Timeline
                    </h3>
                    <span className="text-[10px] font-bold text-[var(--steel)] bg-[var(--sunk)] px-2 py-0.5 rounded border border-[var(--line)]">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar mt-4 space-y-3 pr-1">
                    {result.coordinates && result.coordinates.length > 0 ? (
                      result.coordinates.map((hl, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 text-[10px] text-[var(--steel)] font-semibold">
                              <ClockIcon />
                      ...
                      <div className="text-center py-8 text-xs text-[var(--fog)] font-semibold">
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between border-b border-[var(--line)] pb-3.5">
                  ...
                  <span className="text-[10px] font-bold text-[var(--steel)] bg-[var(--sunk)] px-2.5 py-1 rounded border border-[var(--line)]">
                ...
                {result.clips && result.clips.length > 0 ? (
                    ...
                    {result.clips.map((clipUrl, i) => (
                      ...
                      >
                        <div className="relative group/vid border-b border-[var(--line)] overflow-hidden bg-[var(--night)]">
                          <video
                            src={clipUrl}
                            className={`w-full ${getAspectClass(result.aspectRatio)} object-cover bg-[var(--night)] hover:opacity-85 transition-opacity`}
                            controls={false}
                            loop
                            muted
                            playsInline
                        ...
                        <div className="p-3 bg-[var(--surface)] backdrop-blur-sm border-t border-[var(--line)] flex-1 flex flex-col justify-between gap-2">
                          {result.prompt && (
                            <p className="text-[var(--iron)] text-xs line-clamp-2 leading-relaxed" title={result.prompt}>
                              {result.prompt}
                            </p>
                          )}
                          <div className="flex items-center justify-between mt-1">
                  ...
                  <div className="py-20 text-center text-xs text-[var(--fog)] font-semibold border border-[var(--line)] rounded bg-[color-mix(in_srgb,var(--void)_20%,transparent)]">
